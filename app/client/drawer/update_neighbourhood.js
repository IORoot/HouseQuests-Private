...
export function update_neighbourhood(propertyStatistics)
{
    ...
    if (propertyStatistics.mortgageWidgetModel.currentAverage){
        ...
        var rating = 100 - ((100 / currentAverage) * propertyPrice)
        output_neighbourhood_row({
            ...
            'rating':       percentageToRating(rating)
        })
    ...
    if (propertyStatistics.mortgageWidgetModel.percentageChange){
        ...
        output_neighbourhood_row({
            'attribute':    propertyStatistics.mortgageWidgetModel.percentageChange,
    ...
    if ( propertyStatistics.mortgageWidgetModel.currentAverage )
    {
        ...
        output_neighbourhood_row({
            'attribute':    '£'+pricechange.toFixed(0),
            'icon':         '',
            'target':       'neighbourhood-results-table',
            'title':        '',
            'guide':        '',
    ...
    if (propertyStatistics.results.resultCount){
        ...
        switch (true) {
            case (resultCount < 100):
                rating = '<span class="text-green-400">Great</span>';
                break;
            case (resultCount < 500):
                rating = '<span class="text-emerald-400">Good</span>';
                break;
            case (resultCount < 1000):
                rating = '<span class="text-orange-400">OK</span>';
                break;
            case (resultCount < 2000):
                rating = '<span class="text-red-400">High</span>';
                break;
            case (resultCount > 2000):
                    rating = '<span class="text-red-700">Extreme</span>';
