...
export function listener_show_supermarkets()
{

    const propertyShopsButton = document.getElementById('load-local-supermarkets');
    ...
    propertyShopsButton.addEventListener('click', async function(event) {
        ...
        query += 'way["shop"="supermarket"]('+south+','+west+','+north+','+east+');'
        query += 'relation["shop"="supermarket"]('+south+','+west+','+north+','+east+');'
        query += ');'
        query += 'out;>;out skel qt;'

        const openpassResults = await request_overpass_api(openpassBoundingBox, query)

        load_openpass_results('local-supermarkets', openpassResults, "rgba(255, 255, 255, 1.0)", "rgba(0, 0, 0, 1.0)")
