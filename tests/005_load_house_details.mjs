...
describe('Test Loading single house from Zoopla', () => {
    it('should load data from zoopla and display a single house', async () => {
        ...
        // drawer-title
        let title = await $('#drawer-title').getText()
        console.log('title:', title, ' price:', price)
        await expect(title).not.toBeNull();

        // drawer-price
        let price = await $('#drawer-price').getText()
        await expect(price).not.toBeNull();

        // drawer-contents
        let contents = await $('#drawer-contents').getText()
        await expect(contents).not.toBeNull();
    })
})
