...
export async function request_property_broadband(longitude, latitude)
{
    ...
    if (!broadbandResult){
        message += '<div class="bg-yellow-500 text-white fill-white rounded-lg p-4 flex flex-row gap-2">'
            ...
            message += '<a class="ml-auto text-xs hover:underline" href="https://propertychecker.co.uk/" target="_blank">source</a>'
        message += '</div>'
    }
        ...
        message += '<div class="bg-emerald-500 text-white fill-white rounded-lg p-4 flex flex-col gap-1">'
            ...
            message += '</div>'
                ...
                message += '<tbody>'
                    ...
                    message += '<th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">Max Download Speed</th>'
                    message += '<td class="py-4 px-6">' + broadbandResult.broadbandSpeed.maxDownloadSpeed +'</td>'
                    message += '</tr>'
                    message += '<tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">'
                    message += '<th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">Max Upload Speed</th>'
                    message += '<td class="py-4 px-6">' + broadbandResult.broadbandSpeed.maxUploadSpeed +'</td>'
                    message += '</tr>'
                    message += '<tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">'
                    message += '<th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">Superfast Available</th>'
                    message += '<td class="py-4 px-6">' + (broadbandResult.broadbandSpeed.superfastAvailable  ? 'Yes' : 'No') +'</td>'
                    message += '</tr>'
                    message += '<tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">'
                    message += '<th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">Ultrafast Available</th>'
